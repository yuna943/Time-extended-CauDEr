...
rollback_all_delta(#system{pool = Pool, clock = Clock} = Sys) ->
	Procs = cauder_pool:to_list(Pool),
	PreClock = Clock,
	DeltaTimes =
        [Time ||
            #process{
                hist = [#hist_delta{delay = Time} | _]
            } <- Procs],

	io:format("DeltaTimes: ~p~n", [DeltaTimes]),
    ...
    case DeltaTimes of
        ...
        _ ->
            NewClock = PreClock - lists:min(DeltaTimes),
            N_NewSys = NewSys#system{clock = NewClock},
			N_NewSys
	end.

rollback_one_delta(P, Sys0) ->
    case Hist0 = P#process.hist of
        [#hist_delta{env = Bs, expr = Es, stack = Stk, delay = Delay} | _] ->
            Pool0 = Sys0#system.pool,
			Timer = P#process.timer,
            ...
            Sys1 = Sys0#system{
                pool = cauder_pool:update_with(
                    P#process.pid,
                    fun(Proc) ->
                        Proc#process{hist = RestHist, timer = Timer - Delay}
                    end,
                    Pool0
                )
            },
...
rule_to(
	Pid, 
	#hist_to{env = Bs, expr = Es, stack = Stk}= Entry, 
...
rule_receive(
    ...
    P1 = P0#process{
		...
		timer = Entry#hist_receive.timer
    },
