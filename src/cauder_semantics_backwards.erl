...
%% API
-export([exists_non_delta/1, step_manual/2, step_automatic/2, options/1]).
...
%% the given System.
-spec step_manual(Pid, System) -> NewSystem when
	Pid :: cauder_process:id(),
	System :: cauder_system:system(),
	NewSystem :: cauder_system:system().

step_manual(Pid, #system{pool = Pool} = Sys0) ->
	Proc = cauder_pool:get(Pid, Pool),
	Hist = Proc#process.hist,
	case exists_non_delta(Sys0) of
		false -> rollback_all_delta(Sys0);   %全てのプロセスにおいて、ヒストリーの先頭がdeltaである場合は、全てのプロセスをdeltaでロールバック
		true -> 
			case Hist of
				[] ->
					Sys0;  %ヒストリーが空の場合は、ロールバックできないため、元のシステムを返す
				[Entry|_] ->
					case Entry of
						#hist_delta{} ->
							Sys0;
...
-spec step_automatic(Pid, System) -> NewSystem when
    Pid :: cauder_process:id(),
    System :: cauder_system:system(),
    NewSystem :: cauder_system:system().

step_automatic(Pid, #system{pool = Pool} = Sys0) ->
	case exists_non_delta(Sys0) of
		false -> rollback_all_delta(Sys0);   %全てのプロセスにおいて、ヒストリーの先頭がdeltaである場合は、全てのプロセスをdeltaでロールバック
		true -> 
			Proc = cauder_pool:get(Pid, Pool),
			...
			case Hist of
				...
				[Entry|_] ->
					case Entry of
						#hist_delta{} ->
							rollback_one_nondelta(Sys0);  %ヒストリーの先頭がdeltaである場合は、deltaでないプロセスを1ステップロールバック
						_ ->
							{{value, Entry}, RestHist} = cauder_history:pop(Hist),
							Sys = Sys0#system{
								pool = cauder_pool:update_with(
...
%-spec rule_delta(Pid, Entry, System) -> NewSystem when
%    Pid :: cauder_process:id(),
%    Entry :: cauder_history:entry_tau(),
%    System :: cauder_system:system(),
%    NewSystem :: cauder_system:system().

%rule_delta(Pid, #hist_delta{env = Bs, expr = Es, stack = Stk}, Sys) ->
%	case  exists_non_delta(Sys) of
%		true -> rollback_one_nondelta(Sys);  %ヒストリーの先頭がdeltaでないプロセスが存在する場合は、deltaでないプロセスを1ステップロールバック
%		false -> rollback_all_delta(Sys)   %全てのプロセスにおいて、ヒストリーの先頭がdeltaである場合は、全てのプロセスをdeltaでロールバック
%	end.

exists_non_delta(Sys) ->
	Procs = cauder_pool:to_list(Sys#system.pool),
	lists:any(fun(P) -> not history_starts_with_delta(P) end, Procs).

history_starts_with_delta(P) ->
	io:format("history_starts_with_delta: ~p~n", [P#process.hist]),
	case P#process.hist of
		[H|_] ->
			element(1, H) =:= hist_delta;
		_ -> false
	end.

rollback_one_nondelta(Sys) ->
	io:format("rollback_one_nondelta~n"),
	Procs = cauder_pool:to_list(Sys#system.pool),
	case 
		lists:dropwhile(fun(P) -> history_starts_with_delta(P) end, Procs) of
			[P|_] -> rollback_one_step(P, Sys);
			[] -> Sys
	end.

rollback_one_step(P, Sys0) ->
    io:format("rollback_one_step: ~p~n", [P#process.pid]),
    ...
    case P#process.hist of
        ...
        Hist ->
            %% ① pop
            {{value, Entry}, RestHist} =
                cauder_history:pop(Hist),

            %% ② pool 更新
            Pool0 = Sys0#system.pool,
            Sys1 = Sys0#system{
                pool = cauder_pool:update_with(
                    P#process.pid,
                    fun(Proc) ->
                        Proc#process{hist = RestHist}
                    end,
                    Pool0
                )
            },
            ...
            %% ③ rollback rule
            rollback_entry(P#process.pid, Entry, Sys1)
    end.

rollback_entry(Pid, Entry, Sys) ->
...
rollback_all_delta(Sys) ->
	io:format("rollback_all_delta~n"),
	Procs = cauder_pool:to_list(Sys#system.pool),
	NewSys = 
		lists:foldl(
			fun(P, AccSys) ->
				rollback_one_delta(P, AccSys)
			end,
			Sys,
			Procs),
	NewSys.

rollback_one_delta(P, Sys0) ->
    case P#process.hist of
        [#hist_delta{env = Bs, expr = Es, stack = Stk} | _] ->
            %% 最新 pool を取得
            Pool0 = Sys0#system.pool,

            %% 念のため pool から最新 process を取得
            #process{hist = Hist0} =
                cauder_pool:get(P#process.pid, Pool0),

            %% pop
            {{value, _Entry}, RestHist} =
                cauder_history:pop(Hist0),
            ...
            %% delta の rollback
            rule_simple(P#process.pid, {Bs, Es, Stk}, Sys1);

        _ ->
            Sys0
    end.

-spec rule_to(Pid, Entry, System) -> NewSystem when
    Pid :: cauder_process:id(),
    Entry :: cauder_history:entry_tau(),
    System :: cauder_system:system(),
    NewSystem :: cauder_system:system().

rule_to(Pid, #hist_to{env = Bs, expr = Es, stack = Stk}, Sys) ->
    rule_simple(Pid, {Bs, Es, Stk}, Sys).
