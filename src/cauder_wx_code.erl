...
update_expression(#wx_state{system = System, pid = Pid},
                  #wx_state{system = System, pid = Pid}) ->
    ok;
...
update_expression(_, #wx_state{system = #system{pool = Pool}, pid = Pid}) ->
    ExpressionControl =
        cauder_wx:find(?CODE_Expression_Control, wxTextCtrl),
    Proc = cauder_pool:get(Pid, Pool),
	case Proc#process.expr of
		[] ->
