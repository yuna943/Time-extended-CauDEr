...
-type af_receive() :: {'receive', line(), af_clause_seq()}|{'receive', line(), af_clause_seq(),{'after', abstract_expr(), af_body()}}.
...
expr({'receive', Anno, Cs0}) ->
    Cs1 = icr_clauses(Cs0),
    {'receive', ln(Anno), Cs1};
expr({'receive', Anno, Cs0, T0, E0}) ->
    Cs1 = icr_clauses(Cs0),
    T   = expr(T0),
    E   = exprs(E0),
    {'receive', ln(Anno), Cs1, {'after', T, E}};
...
replace_variable({'receive', Line, Cs0}, Var, Val) ->
    Cs = replace_variable(Cs0, Var, Val),
    {'receive', Line, Cs};
replace_variable({'receive', Line, Cs0, {'after', T0, E0}}, Var, Val) ->
    Cs = replace_variable(Cs0, Var, Val),
	T  = replace_variable(T0,  Var, Val),
	E  = replace_variable(E0,  Var, Val),
    {'receive', Line, Cs ,{'after', T, E}};
...
to_abstract_expr({'receive', Line, Cs}) ->
    ...
    set_line(Node, Line);
to_abstract_expr({'receive', Line, Cs, {'after', T, E}}) ->
    Node =
