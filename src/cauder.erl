...
step(Sem, Scheduler, Pid, Sys, Steps) ->
        ...
        case Sem of
            ?SEM_FWD ->
                fun(Step, Sys0) ->
					...
					Procs = cauder_pool:to_list(Sys0#system.pool),
					Waiting =  %waitingしているプロセスの集合
						[P || P <- Procs, P#process.status =:= waiting, P#process.is_alive =:= true],
					Runnable = %runnningしているプロセスの集合
						[P || P <- Procs, P#process.status =:= runnable,P#process.is_alive =:= true],

					SizeOpts = maps:size(Opts),
					SizeWaiting = length(Waiting),
					SizeRunnable = length(Runnable),

					%io:format("[step/5 DEBUG] maps:size(Opts) = ~p , Waiting = ~p, Runnable = ~p~n", [SizeOpts, SizeWaiting, SizeRunnable]),
					case {SizeOpts, SizeWaiting, SizeRunnable} of
						{0, 0, 0} ->
							% 全員 finished
							%io:format("[step/5 DEBUG] all finished~n"),
							throw({success, Sys0, Step});
						{0, _, 0} -> %全員 waiting
							%io:format("[step/5 DEBUG] all waiting, advance time Step=~p~n", [Step]),
							%throw({retry, advance_time(Sys0), Step});
							Sys1 = advance_time(Sys0),
							Sys2 = handle_timeout(Sys1),
							throw({success, Sys2, Step + 1});
						{0,_, _} ->
							% runnable プロセスはいるが、適用可能な rule がない
    						% → stuck 状態なので step を終了
							%io:format("[step/5 DEBUG] stuck state, no applicable rule~n"),
							throw({success, Sys0, Step});
						_ ->
							%io:format("[step/5 DEBUG] normal step execution~n"),
							case maps:is_key(Pid, Opts) of
								false ->
									%io:format("[step/5 DEBUG] go false~n"),
									throw({success, Sys0, Step});
								true ->
									%io:format("[step/5 DEBUG] go true~n"),
                ...
                fun(Step, Sys0) ->
                    ...
                    case maps:is_key(Pid, Opts) of
                        false ->
                            throw({success, Sys0, Step});
                        true ->
                            cauder_semantics_backwards:step_manual(Pid, Sys0)
                    end
                end
        end,
    try lists:foldl(Fun, Sys, lists:seq(0, Steps - 1)) of
		...
		throw:{retry, Sys1, Step0} ->
			%% 時間を進めたので、同じ step から再開
			step(Sem, Scheduler, Pid, Sys1, Steps - Step0);

		throw:{success, Sys1, Step0} ->
			{success, Sys1, Step0}
	end.

-spec advance_time(System) -> NewSystem when
    System :: cauder_system:system(),
    NewSystem :: cauder_system:system().

advance_time(#system{pool = Pool, clock = Clock} = Sys) ->
	%io:format("[advance_time DEBUG] advance_time called~n"),
    Procs = cauder_pool:to_list(Pool),
	%io:format("[advance_time DEBUG] processes: ~p~n", [Procs]),

    %% waiting かつ有限 timeout のプロセス
    Timed =
        [P || P <- Procs,
            P#process.status =:= waiting,
            P#process.timeout =/= infinity],
	%io:format("[advance_time DEBUG] Timed processes: ~p~n", [Timed]),
    ...
    case Timed of
        ...
        _ ->
            Tmin = lists:min([P#process.timeout || P <- Timed]),
			NewClock = Clock + Tmin,
            NewPool =
                lists:foldl(
                    fun(P, AccPool) ->
						NewP = 
							case {P#process.status, P#process.timeout} of
								{waiting, T} when T =:= Tmin ->
									%io:format("[advance_time DEBUG] timeout fired for process ~p~n", [P#process.pid]),
									HistEntry1 = #hist_delta{
										env = P#process.env,
										expr = P#process.expr,
										stack = P#process.stack,
										time = Tmin
									},
									...
									P#process{
										status = runnable,
										timeout = T - Tmin,
										hist = cauder_history:push(HistEntry1, P#process.hist)
										%expr = fire_after_expr(P#process.after_body,P#process.expr),
										%after_body = none
									};

									%HistEntry2 = #hist_to{
									%	env = P1#process.env,
									%	expr = P1#process.expr,
									%	stack = P1#process.stack
									%},

									%P1#process{
									%	hist = cauder_history:push(HistEntry2, P1#process.hist)
									%};
								...
								{_, T} when T > Tmin ->
									...
									case T of 
										infinity ->
											...
											P#process{
												hist = cauder_history:push(HistEntry, P#process.hist)
											};
										_ ->
											HistEntry = #hist_delta{
												env = P#process.env,
						...
						cauder_pool:update(NewP, AccPool)
                    end,
					Pool,
                    Procs
                ),
            Sys#system{pool = NewPool, clock = NewClock}
    end.
-spec handle_timeout(System) -> NewSystem when
    System :: cauder_system:system(),
    NewSystem :: cauder_system:system().

handle_timeout(#system{pool = Pool} = Sys) ->
	%io:format("[handle_timeout DEBUG] handle_timeout called~n"),
	Procs = cauder_pool:to_list(Pool),
    NewPool =
        lists:foldl(
            fun(P, AccPool) ->
                case P#process.timeout of
                    0 ->
                        %% timeout発火
                        HistEntry = #hist_to{
                            env = P#process.env,
    ...
    Sys#system{pool = NewPool}.

-spec fire_after_expr([term()],[term()]) -> [term()].
%fire_after_expr(
%		[{value, _, ok}],[{'receive', _Line, _Clauses,{'after', _Timeout, AfterBody}}| Rest]
%	) ->
%		Rest;
fire_after_expr([Expr1], [{'receive', _Line, _Clauses,{'after', _Timeout, AfterBody}}| Rest]) ->
    [Expr1 | Rest];
%fire_after_expr(
%	[{value, _, ok}],[{remote_call, _, timer, sleep, [TimeExpr]}|Rest]
%) ->
	%io:format("[fire_after_expr DEBUG] fire_after_expr called~n"),
%    Rest;
fire_after_expr([Expr1], [{remote_call, _, timer, sleep, [TimeExpr]}|Rest]) ->
    [Expr1 | Rest];
fire_after_expr([_], [Exprs]) ->
	[Exprs].
...
step_multiple(Sem, Scheduler, Sys, Steps) ->
	%io:format("[step_multiple DEBUG] step_multiple called, steps = ~p~n", [Steps]),
    ...
    DoStep =
        fun(Step, {Sys0, Set0, PidQueue0}) ->
            Opts =
                case Sem of
                    ?SEM_FWD -> 
						%io:format("[step_multiple DEBUG] options/5 called~n"),
						cauder_semantics_forwards:options(Sys0, normal);
			...
			SizeWaiting = length(Waiting),
			SizeRunnable = length(Runnable),
			%io:format("[step_multiple DEBUG] maps:size(Opts) = ~p , Waiting = ~p, Runnable = ~p~n", [SizeOpts, SizeWaiting, SizeRunnable]),
			case {SizeOpts, SizeWaiting, SizeRunnable} of
                {0, 0, 0} ->
					% 全員 finished
					%io:format("[step_multiple DEBUG] all finished~n"),
					throw({success, Sys0, Step});
				{0, _, 0} -> %全員 waiting
					% 時間を進めたあと、同じ Step でもう一度続行
					%io:format("[step_multiple DEBUG] all waiting, advance time~n"),
					%throw({retry, advance_time(Sys0), Step});
					Sys1 = advance_time(Sys0),
					Sys2 = handle_timeout(Sys1),
					%throw({retry, Sys2, Step});
					throw({success, Sys2, Step + 1});
				{0,_, _} ->
					% runnable プロセスはいるが、適用可能な rule がない
					% → stuck 状態なので step を終了
					%io:format("[step_multiple DEBUG] stuck state, no applicable rule~n"),
					throw({success, Sys0, Step});
                _ ->
					%io:format("[step_multiple DEBUG] normal step execution~n"),
					Pids = maps:keys(Opts),
                    Set1 = sets:from_list(Pids),
                    Change =
                        case {sets:size(Set0), sets:size(Set1)} of
                            {0, _} ->
								%io:format("[step_multiple DEBUG] initial step~n"),
                                {init, Pids};
                            {Size0, Size1} when Size0 < Size1 ->
								%io:format("[step_multiple DEBUG] process added~n"),
                                [AddedPid] = sets:to_list(sets:subtract(Set1, Set0)),
                                {add, AddedPid};
                            {Size0, Size1} when Size0 > Size1 ->
								%io:format("[step_multiple DEBUG] process removed~n"),
                                [RemovedPid] = sets:to_list(sets:subtract(Set0, Set1)),
                                {remove, RemovedPid};
                            {Size, Size} ->
								%io:format("[step_multiple DEBUG] process updated~n"),
                                case
                                    {
					...
					%io:format("[step_multiple DEBUG] selected pid ~p~n", [Pid]),
                    Sys1 =
                        case Sem of
                            ?SEM_FWD -> cauder_semantics_forwards:step(Pid, Sys0, ?SCHEDULER_Random, normal);
                            ?SEM_BWD -> cauder_semantics_backwards:step_automatic(Pid, Sys0)
                        end,
                    {Sys1, Set1, PidQueue1}
            end
        end,
...
%        throw:{Sys1, StepsDone} -> {Sys1, StepsDone};
		throw:{retry, Sys1, Step0} ->
			%% 時間を進めたので、同じ step から再開
			%io:format("[step_multiple DEBUG] Step0=~p, Steps=~p~n", [Step0, Steps]),
			step_multiple(Sem, Scheduler, Sys1, Steps - Step0);
