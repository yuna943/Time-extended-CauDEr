...
advance_time(#system{pool = Pool, clock = Clock} = Sys) ->
    ...
    case Timed of
        ...
        _ ->
            Tmin = lists:min([P#process.timeout - P#process.timer || P <- Timed]),
			io:format("[advance_time DEBUG] Tmin = ~p~n", [Tmin]),
			NewClock = Clock + Tmin,
            NewPool =
                lists:foldl(
                    fun(P, AccPool) ->
						NewP = 
							case P#process.timeout of
								infinity ->
									...
									NewTimer = P#process.timer,
									P#process{
										timer = NewTimer + Tmin,
										hist = cauder_history:push(HistEntry, P#process.hist)
									};
								_ ->
									case {P#process.status, P#process.timeout - P#process.timer} of
...
handle_timeout(#system{pool = Pool} = Sys) ->
    ...
    NewPool =
        lists:foldl(
            fun(P, AccPool) ->
                case P#process.timer == P#process.timeout of
                    true ->
                        HistEntry = #hist_to{
                            env = P#process.env,
                            expr = P#process.expr,
                            stack = P#process.stack,
							timeout = P#process.timeout
                        },
