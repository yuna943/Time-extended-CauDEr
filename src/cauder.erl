...
step(Sem, Scheduler, Pid, Sys, Steps) ->
	%io:format("[step/5 DEBUG] step/5 called Pid=~p, Steps=~p~n", [Pid, Steps]),
    Fun =
        case Sem of
            ?SEM_FWD ->
                fun(Step, Sys0) ->
					%io:format("[step/5 DEBUG] options/5 called~n"),
...
% -spec can_advance_time(
% 	System ::cauder_system:system()
% ) -> boolean().

% can_advance_time(#system{pool = Pool}) ->
% 	lists:all(
% 		fun(P) ->
% 			P#process.is_alive =:= true andalso
% 			P#process.status =:= waiting andalso
% 			P#process.timeout =/= 0
% 		end,
...
advance_time(#system{pool = Pool} = Sys) ->
						...
						NewP = 
							case {P#process.status, P#process.timeout} of
								{waiting, T} when T =:= Tmin ->
									%% after 発火
									%io:format("[advance_time DEBUG] 'after' fired pid~p~n",[P#process.pid]),
									%io:format("[advance_time DEBUG] after_body = ~p~n",[P#process.after_body]),
									%io:format("[advance_time DEBUG] expr = ~p~n",[P#process.expr]),
