...
-export([process/2, log_action/1, history_entry/1, stack_entry/1, expression/1, trace_action/1]).
...
-spec process(Process,System) -> nonempty_string() when
    Process :: cauder_process:process(),
    System :: cauder_system:system().

process(#process{node = Node, pid = Pid, mfa = {M, F, A}, hist = Hist, status = Sta} = Proc, #system{mail= Mail} = Sys) ->
	HasMessage = 
		case cauder_mailbox:find_destination(Pid, Mail) of
			[] -> false;
			_ -> true
		end,
	%io:format("HasMessage: ~p~n", [HasMessage]),
    Icon =  %アイコン設定
		case {Sta,Hist,cauder_process:is_alive(Proc),HasMessage} of
			{_,_,false,_} -> ?ICON_DEAD;
			{waiting, _,true,true} -> ?ICON_ALIVE;
			{_,[#hist_delta{}|_],_,_} -> ?ICON_TIME;
			{_,[#hist_to{}|_],_,_} -> ?ICON_TIMEOUT;
			{waiting, _,_,false} -> ?ICON_TIME;
			{_,_,true,_} -> ?ICON_ALIVE
		%case Sta of
		%	waiting -> ?ICON_TIME;
		%	_ ->
