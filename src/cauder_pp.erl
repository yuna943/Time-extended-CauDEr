...
process(#process{node = Node, pid = Pid, mfa = {M, F, A}, hist = Hist, status = Sta} = Proc) ->
    Icon =  %アイコン設定
		case {Sta,Hist,cauder_process:is_alive(Proc)} of
			{_,_,false} -> ?ICON_DEAD;
			{runnable,[#hist_delta{}|_],_} -> ?ICON_TIME;
			{runnable,[#hist_to{}|_],_} -> ?ICON_TIMEOUT;
			{waiting, _,_} -> ?ICON_TIME;
			{_,_,true} -> ?ICON_ALIVE
		%case Sta of
		%	waiting -> ?ICON_TIME;
		%	_ ->
