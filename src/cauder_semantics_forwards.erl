...
%%% system.
%%% @end
%%% このモジュールには 2 つの関数が含まれており、
%%% 1 つは与えられたシステムに対して評価オプションを取得する関数、
%%% もう 1 つは与えられたシステムのあるプロセスにおいて評価ステップを実行する関数です。
%%%-----------------------------------------------------------------------------
...
step(Pid, Sys, Sched, Mode) ->
    ...
    case Result#result.label of
        ...
        #label_tau{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_local~n", [Pid]),
            rule_local(Pid, Result, Sys);
        #label_self{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_self~n", [Pid]),
            rule_self(Pid, Result, Sys);
        #label_node{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_node~n", [Pid]),
            rule_node(Pid, Result, Sys);
        #label_nodes{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_nodes~n", [Pid]),
            rule_nodes(Pid, Result, Sys);
        % consider the map contains ghost
        #label_registered{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_registered~n", [Pid]),
            rule_registered(Pid, Result, Sys);
        % consider the map contains ghost
        #label_whereis{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_whereis~n", [Pid]),
            rule_whereis(Pid, Result, Sys);
        % consider the map contains ghost
        #label_register{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_register~n", [Pid]),
            rule_register(Pid, Result, Sys);
        % consider the map contains ghost
        #label_unregister{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_unregister~n", [Pid]),
            rule_unregister(Pid, Result, Sys);
        #label_start{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_start~n", [Pid]),
            rule_start(Pid, Result, Sys);
        #label_spawn_fun{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_spawn~n", [Pid]),
        ...
        % consider the map contains ghost
        #label_send{} ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_send~n", [Pid]),
            rule_send(Pid, Result, Sys);
        #label_receive{var = Var} when Result#result.expr == [Var] ->
			%io:format("[step/4 DEBUG] pid=~p applying rule_receive~n", [Pid]),
...
options(#system{pool = Pool, mail = Mail} = Sys, Mode) ->
	Opts = 
		lists:foldl(
			fun(#process{pid = Pid, expr = E, stack = Stk, hist = His, env = Bs, status = Sta, is_alive = Alive, timeout = To}, Map) ->
				io:format("[options DEBUG] #process{pid = ~p, expr = ~p, stack = ~p, hist = ~p, env = ~p, status = ~p, is_alive = ~p, timeout = ~p}~n", [Pid, E, Stk, His, Bs, Sta, Alive, To]),
				case Alive of
					false ->
						Map;
					...
					true ->
						%このプロセスの選択肢を確認中
						%io:format("[options DEBUG] checking options for pid=~p timeout=~p~n",[Pid, To]),
						case Sta of
							waiting ->
								%io:format("[options DEBUG] pid=~p is waiting, timeout=~p~n", [Pid, To]),
								case E of
									%% ===== receive =====
									[{'receive', _, Cs, {'after', TimeExpr, AfterBody}} | Rest]->
										case check_reducibility(Cs, Pid, Sys, Mode, 'receive') of
											{'ok', Rule} ->
												%io:format("[options DEBUG] pid=~p is runnning, available rule: ~p~n", [Pid, Rule]),
												maps:put(Pid, Rule, Map);
											_ ->
												%io:format("[options DEBUG] go Map~n"),
												Map
										end;
									[{remote_call, _, timer, sleep, [TimeExpr]}|Rest] ->
										Time = cauder_eval:concrete(TimeExpr),
										case check_reducibility([], Pid, Sys, Mode, 'receive') of
											{'ok', Rule} ->
												%io:format("[options DEBUG] pid=~p is running (timer:sleep as receive), available rule: ~p~n", [Pid, Rule]),
												maps:put(Pid, Rule, Map);
											_ -> 
												%io:format("[options DEBUG] go Map~n"),
												Map
								...
								case E of
									...
									[{'receive', _, Cs, {'after', TimeExpr, AfterBody}} | Rest]->
										case expression_option(E, Pid, Sys, Mode) of
											{'ok', Rule} ->
												%io:format("[options DEBUG] pid=~p is waiting, available rule: ~p~n", [Pid, Rule]),
												maps:put(Pid, Rule, Map);
											_ ->
												Map
										end;
									%% ===== timer:sleep/1 as receive =====
									[{remote_call, _, timer, sleep, [TimeExpr]}|Rest] ->
										Time = cauder_eval:concrete(TimeExpr),
										%io:format("[options DEBUG] pid=~p is timer:sleep(~p)~n", [Pid, Time]),
										%% 擬似 receive として処理
										case Time of
											0 ->
												%% after 発火
												case expression_option([{'receive', [], [], {'after', Time, [{value, [], ok}]}} | Rest], Pid, Sys, Mode) of
													{'ok', Rule} ->
														%io:format("[options DEBUG] pid=~p is runnable (timer:sleep as receive), available rule: ~p~n", [Pid, Rule]),
														maps:put(Pid, Rule, Map);
													_ -> Map
												end;
											_ ->
												%% 論理的に待機中でも「receive」として扱う
												case expression_option([{'receive', [], [], {'after', Time, [{value, [], true}]}} | Rest],Pid, Sys, Mode) of
													{'ok', Rule} ->
														%io:format("[options DEBUG] pid=~p is waiting (timer:sleep as receive), available rule: ~p~n", [Pid, Rule]),
														maps:put(Pid, Rule, Map);
													_ -> Map
												end
										end;
									...
									_ ->
										case expression_option(E, Pid, Sys, Mode) of
											{'ok', Rule} ->
												%io:format("[options DEBUG] pid=~p is runnable, available rule: ~p~n", [Pid, Rule]),
												maps:put(Pid, Rule, Map);
											_ ->
												Map
										end
	...
	%io:format("[options/2 DEBUG] final Opts: ~p~n", [Opts]),
	Opts.
        ...
        case Label of
            #label_spawn_fun{function = {value, Line, Fun} = FunLiteral} ->
                ...
                #process{
					...
					stack = cauder_stack:new(),
					hist = cauder_history:new(),
					is_alive = true,
					status = runnable,
					timeout = infinity,
					after_body = none
...
rule_send(
	...
	%io:format("[rule_send DEBUG] pid ~p send ~n",[Pid]),
...
-spec rule_receive(Pid, Result, Mode, Scheduler, System) -> NewSystem when
    Pid :: cauder_process:id(),  %この receive を実行するプロセスの PID
    Result :: cauder_eval:result(),  %receive のマッチ部分（ガードなど）の情報
    Mode :: normal | replay,   %normal or replay（可逆実行用
    Scheduler :: cauder_message:scheduler(),
    System :: cauder_system:system(),
    NewSystem :: cauder_system:system().   %システム（メールボックス、プロセスプール、トレース、ログ）

rule_receive(
    Pid,
    #result{env = Bs, label = #label_receive{clauses = Cs, timeout = To}} = Result,
    Mode,
    Sched,
    #system{mail = Mail0, pool = Pool, log = Log0} = Sys
) ->
	%io:format("[rule_receive DEBUG]pid ~p recieve,  To = ~p~n", [Pid, To]),
    {Timeout, AfterBody} =
        case To of
            none -> {none, none};
            infinity -> {infinity, none};
%			{T, [{_,_,ok}|Rest]} -> {cauder_eval:concrete(T), none};
            {T, Ab} -> {cauder_eval:concrete(T), Ab}
        end,
	 %デバッグ用のログ
    %io:format("[rule_receive DEBUG]RECEIVE pid=~p status=~p timeout=~p mail=~p~n",[Pid,(cauder_pool:get(Pid, Pool))#process.status,(cauder_pool:get(Pid, Pool))#process.timeout,Mail0]),
    %% ===== マッチ部分（読むだけ）=====
    MatchResult =
        case Mode of
            normal ->
                case cauder_eval:match_rec_pid(Cs, Bs, Pid, Mail0, Sched, Sys) of
                    nomatch ->
                        nomatch;
                    Match ->
                        {_, _, {#message{uid = Uid} = Msg_new, _}, _} = Match,
                        Log_new =
                            case cauder_log:pop_receive(Pid, Log0) of
                                {#log_receive{uid = Uid}, NewLog} ->
                                    NewLog;
                                _ ->
                                    cauder_log:rdep(Pid, Log0)
                            end,
                        {Match, Log_new, Msg_new}
                end;

            replay ->
                {#log_receive{uid = Uid}, Log_new} =
                    cauder_log:pop_receive(Pid, Log0),
                case cauder_eval:match_rec_uid(Cs, Bs, Uid, Mail0) of
                    nomatch ->
                        nomatch;
                    Match ->
                        {_, _, {Msg_new, _}, _} = Match,
                        {Match, Log_new, Msg_new}
                end
        end,
    ...
    case MatchResult of
        nomatch ->
			...
			%io:format("[rule_receive DEBUG] pid=~p timeout=~p~n",[Pid,Timeout]),
			NewSys = 
				case P0#process.status of
					%% ① すでに waiting → 何もしない（advance_time に任せる）
					waiting ->
						Sys;
					%% ② その他
					_ ->
						case Timeout of
							0 ->
								%% timeout 発火 → runnable に
								NewProc = P0#process{status = runnable},
								Sys#system{pool = cauder_pool:update(NewProc, Pool)};
							none ->
								mark_waiting(Pid, infinity, none, Sys);
							infinity ->
								mark_waiting(Pid, infinity, none, Sys);
							Tint when is_integer(Tint) ->
								mark_waiting(Pid, Tint, AfterBody, Sys)
						end
				end,
			NewSys;

        {{Bs1, Es1, _, _}, Log1, Msg} ->
            %% ★ mailbox から削除（正式API）
            {QPos, Mail1} = cauder_mailbox:remove(Msg, Mail0),
...
-spec mark_waiting(Pid, Timeout, AfterBody,System) -> NewSystem when 
	Pid :: cauder_process:id(), 
	Timeout :: none | infinity | non_neg_integer(), 
	AfterBody :: none | cauder_syntax:af_body(), 
	System :: cauder_system:system(), 
	NewSystem :: cauder_system:system().

mark_waiting(Pid, To, Ab, #system{pool = Pool} = Sys) ->
	%io:format("[mark_waiting DEBUG] pid=~p timeout=~p after=~p~n",[Pid, To, Ab]),
    ...
    case is_reducible(E0, Proc#process.env) of
        ...
        true ->
            case E0 of
				...
				{'receive', _, Cs, {'after', _, _}} ->
					%io:format("[expression_option DEBUG] receive-after detected pid=~p~n",[Pid]),
					{ok, ?RULE_RECEIVE};
                {'receive', _, Cs} ->
					%io:format("[expression_option DEBUG] receive detected pid=~p~n",[Pid]),
					{ok, ?RULE_RECEIVE};
                {bif, _, _, _, As} ->
...
has_matching_receive(Cs, Bs, Pid, Mail, _Sys) ->
	case Cs of
		[] ->
			true;
		_ ->
				...
				QueueList ->
					lists:any(
						fun(Queue) ->
							lists:any(
								fun(Msg) ->
									cauder_eval:match_rec(Cs, Bs, Msg) =/= nomatch
								end,
								queue:to_list(Queue)
							)
						end,
