...
update_process(_, #wx_state{system = undefined}) ->
    ...
    ok;
update_process(#wx_state{pid = OldPid}, #wx_state{system = System = #system{pool = Pool}}) ->
    Choice = cauder_wx:find(?ACTION_Process, wxChoice),
    ...
    {_, NewIdx} =
        lists:foldl(
            fun(P, {Idx, Match}) ->
                Label = cauder_pp:process(P,System),
                Pid = P#process.pid,
...
create_manual(Parent) ->
	...
	Clocks = wxBoxSizer:new(?wxHORIZONTAL),
    wxBoxSizer:add(Content, Clocks, CenterHorizontal),

	ClockText = wxStaticText:new(Win, ?wxID_ANY, "Time:", StaticAlignRight),
	wxBoxSizer:add(Clocks, ClockText, CenterVertical),

	ClockValue = wxStaticText:new(Win, ?CLOCK_Value_Man, "0"),
	wxBoxSizer:add(Clocks, ClockValue, CenterVertical),
...
update_manual(_, #wx_state{system = System, pid = Pid}) ->
	...
	Procs = cauder_pool:to_list(System#system.pool),
	Proc = case lists:keyfind(Pid, #process.pid, Procs) of
		false -> undefined;
		P -> P
	end,
	
	IsAlive = case Proc of
		undefined -> false;
		Proc1 -> cauder_process:is_alive(Proc1)
	end,

	Pool  = System#system.pool,
	Proc_b = cauder_pool:get(Pid, Pool),
	Hist = Proc_b#process.hist,
	
	NotIsTime = case Hist of
		[#hist_delta{}|_] -> false;
		_ -> true
	end,

	EveryTime = case cauder_semantics_backwards:exists_non_delta(System) of
		false -> true;
		_ -> false
	end,

    CanFwd = IsAlive orelse maps:is_key(Pid, OptsFwd),
    CanBwd = (NotIsTime orelse EveryTime) andalso maps:is_key(Pid, OptsBwd),
	...
	ClockValue = cauder_wx:find(?CLOCK_Value_Man, wxStaticText),
	wxStaticText:setLabel(ClockValue, integer_to_list(System#system.clock)),
...
create_automatic(Parent) ->
	...
	ClockValue = wxStaticText:new(Win, ?CLOCK_Value_Aut, "0"),
	wxBoxSizer:add(Clocks, ClockValue, CenterVertical),
...
update_automatic(_, #wx_state{system = System}) ->
	...
	ClockValue = cauder_wx:find(?CLOCK_Value_Aut, wxStaticText),
	wxStaticText:setLabel(ClockValue, integer_to_list(System#system.clock)),
	ok.
